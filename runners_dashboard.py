...
DATA_PATH = Path(__file__).with_name("data.parquet")
...
def filter_data(
    ...
    return filtered.dropna(subset=["Performance_seconds", "Event distance (km)"])
...
with st.sidebar:
    ...
    # Dynamic boxplot selectors: choose X (categorical) and Y (numeric)
    # Build categorical options from known categorical cols or string/object dtypes
    cat_options = [
        c
        for c in df.columns
        if c in CATEGORICAL_COLS or df[c].dtype.name in ("string", "object", "category")
    ]
    if not cat_options:
        cat_options = BOXPLOT_OPTIONS
    box_x_col = st.selectbox("עמודת X (קטגורית) לבוקספלוט", cat_options, index=0)

    # Build numeric options from known numeric cols or numeric dtypes
    num_options = [c for c in df.columns if c in NUMERIC_COLS or pd.api.types.is_numeric_dtype(df[c])]
    # Ensure the convenience performance columns are present as options
    for perf in ["Performance_hours", "Performance_minutes", "Performance_seconds", "Event distance (km)", "age"]:
        if perf in df.columns and perf not in num_options:
            num_options.append(perf)
    # Default to Performance_hours if available
    default_y_index = num_options.index("Performance_hours") if "Performance_hours" in num_options else 0
    box_y_col = st.selectbox("עמודת Y (נומרית) לבוקספלוט", num_options, index=default_y_index)
...
with tab2:
    scatter_df = sample_for_scatter(filtered_df, sample_size)
    st.caption(
    ...
    fig_scatter = px.scatter(
        ...
        title="גרף פיזור: מרחק מול זמן ביצוע",
        opacity=0.5,
    )
    fig_scatter.update_layout(height=560, xaxis_title="מרחק (ק\"מ)", yaxis_title="זמן (שעות)")
    st.plotly_chart(fig_scatter, use_container_width=True)

    # Build boxplot from selected X and Y
    if box_x_col not in filtered_df.columns or box_y_col not in filtered_df.columns:
        st.warning("בחר עמודות תקינות לבוקספלוט.")
    else:
        plot_df = filtered_df.dropna(subset=[box_x_col, box_y_col]).copy()
        # Ensure categorical x is string for consistent plotting
        plot_df[box_x_col] = plot_df[box_x_col].astype(str)
        fig_box = px.box(
            plot_df,
            x=box_x_col,
            y=box_y_col,
            points=False,
            title=f"בוקספלוט {box_y_col} לפי {box_x_col}",
        )
        # Friendly Y axis label for known performance columns
        y_label = box_y_col
        if box_y_col == "Performance_hours":
            y_label = "זמן (שעות)"
        elif box_y_col == "Performance_minutes":
            y_label = "זמן (דקות)"
        fig_box.update_layout(height=560, xaxis_title=box_x_col, yaxis_title=y_label)
...
with tab3:
    ...
    fig = px.bar(
        ...
        barmode="group",
        title=f"השוואה בין קבוצות לפי {compare_col}",
    )
...
with st.expander("על הדשבורד"):
    st.markdown(
        """
        - הדשבורד קורא את הקובץ `data.csv` מאותה תיקייה של הקובץ.
        - בגלל שהקובץ גדול מאוד, גרפי פיזור מבוססים על דגימה כדי לשמור על מהירות תגובה.
        - ניתן להרחיב בקלות את הדשבורד עם מפות, KPI נוספים, פילטרים על מדינות/אירועים וייצוא דוחות.
        """
    )
